...
const uint8_t PinCLK = 2;   // Clock  (decouple Pins 2 and 4 with 0.68uF caps to ground)
const uint8_t PinSW  = 3;   // Push Button Switch
const uint8_t PinDT  = 4;   // Encoder DT Signal
const uint8_t TR_Pin = 8;   // Add T/R switching capability to synthesizer board
...
// Splash screen displayed at power on/reset
void Splash() {
  ...
  oled.clrScreen();
  oled.printLine(0, text1);
  oled.printLine(1, text2);
  oled.printLine(2, text3);
  oled.printLine(3, text4);
}
...
void catstr(char *dst, char *src) {
  uint8_t dstlen= len(dst);
  uint8_t srclen= len(src);
...
void Display_Freq(unsigned long freq) {  // Display Band and Frequency
  ...
  oled.clrScreen();
  char* Bandstring = BandString[Band];
  catstr(Bandstring, "M Freq kHz");
  oled.printLine(0, Bandstring);

  if (EncMode == 0) {  // Second Display Line - Frequency
    oled.setCursor(0, 11);
    oled.putch('.');   // Display dot if Encoder in Frequency mode
  }
